 ...
 * @version    2.0 BETA 2 (Revision: 205, Date: 2008/02/15 05:45:10)
 * @package    Texy
 * @link       http://texy.info/
 */

define('TEXY_VERSION','2.0 BETA 2 (Revision: 205, Date: 2008/02/15 05:45:10)');if(version_compare(PHP_VERSION,'5.1.0','<')){throw
new
Exception('Texy needs PHP 5.1.0 or newer. Older PHP versions are supported by older Texy releases only.');}if(function_exists('mb_get_info')){if(mb_get_info('func_overload')&2&&substr(mb_get_info('internal_encoding'),0,1)==='U'){mb_internal_encoding('pass');trigger_error("Texy: mb_internal_encoding changed to 'pass'",E_USER_WARNING);}}if(ini_get('zend.ze1_compatibility_mode')%
256||preg_match('#on$|true$|yes$#iA',ini_get('zend.ze1_compatibility_mode'))){throw
new
RuntimeException("Texy cannot run with zend.ze1_compatibility_mode enabled.");}if(!class_exists('NObject',FALSE)){abstract
class
NObject{final
public
function
...
TexyImage
extends
NObject{public$URL;public$overURL;public$linkedURL;public$width;public$height;public$asMax;public$modifier;public$name;public
function
...
FALSE;}private
function
checkLink($link){if(strncasecmp($link->URL,'www.',4)===0){$link->URL='http://'.$link->URL;}elseif(preg_match('#'.TEXY_EMAIL.'$#iA',$link->URL)){$link->URL='mailto:'.$link->URL;}elseif(!$this->texy->checkURL($link->URL,Texy::FILTER_ANCHOR)){$link->URL=NULL;}else{$link->URL=str_replace('&amp;','&',$link->URL);}}private
function
...
TexyTableModule
extends
TexyModule{public$oddClass;public$evenClass;public
function
